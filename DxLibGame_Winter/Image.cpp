#include "Image.h"
#include <DxLib.h>
#include <string>
#include "Vector2.h"

Image::Image(const std::string& pass) :
	m_expandRate(1.0f),
	m_angle(0.0f),
	m_blendMode(DX_BLENDMODE_NOBLEND),
	m_blendPal(0)
{
	m_graphHandle = LoadGraph(pass.c_str());
}

Image::~Image()
{
	DeleteGraph(m_graphHandle);
}

void Image::Draw(Vector2 pos) const
{
	SetDrawBlendMode(m_blendMode, m_blendPal);
	DrawRotaGraph(static_cast<int>(pos.x), static_cast<int>(pos.y), static_cast<double>(m_expandRate),
		static_cast<double>(m_angle), m_graphHandle, true);
	SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);
}

void Image::SetGraph(const std::string& pass)
{
	DeleteGraph(m_graphHandle);
	m_graphHandle = LoadGraph(pass.c_str());
}